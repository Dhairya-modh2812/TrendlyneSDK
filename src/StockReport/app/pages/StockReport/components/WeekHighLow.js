...
function WeekHighLow(props) {
    const { priceDistributionData, stockData } = props;
    ...
    let fillerPos = close < startPrice ? +currPos : +estimatePos;
    let fillerWidth = Math.abs(((startPrice - close) / (high - low)) * 100);
    ...
    return ( <>
            <div className='position-relative'>
                <div className="progress price-estimate">
                    <div className={`filler ${color}`} style={{left: `${fillerPos}%`, width: `${fillerWidth + 2}%`}}></div>
                    ...
                    <div className='avg-estimate' style={estimateStyle}>
                        <p className='value'>{startPrice}</p>
                    </div>
