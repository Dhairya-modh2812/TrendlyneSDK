...
function MarketBulkBlockDeals() {
  const dispatch = useDispatch();
  const [bulkTableData, setBulkTableData] = useState([]);
  let n;

  const { superstarName, setSuperstarName , externalView, superstarGroupBulkBlockDealsUrl, corsKey, currentPageSize, setCurrentPageSize, pageSizeOptions = []} = useContext(superstarContext);

  let { id } = useParams();

  useEffect(() => {

    dispatch(superstar_portfolio.getGroupBulkBlockList({externalView, superstarGroupBulkBlockDealsUrl, corsKey})).then((response) => {
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          return (
            <div style={{ textAlign: "center" }}>
              <button
                className={` purchasebutton ${row.original.get_action_display == "Purchase"
                    ? "positive_color"
...
export default MarketBulkBlockDeals;
