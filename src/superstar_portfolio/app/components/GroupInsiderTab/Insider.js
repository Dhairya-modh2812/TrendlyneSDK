...
function MarketInsider() {
  const dispatch = useDispatch();
  const [insiderData, setInsiderData] = useState([]);

  const { superstarName, setSuperstarName, externalView, superstarGroupInsiderTradingSastUrl, corsKey, currentPageSize, setCurrentPageSize, pageSizeOptions = [] } = useContext(superstarContext);
  ...
  useEffect(() => {
    ...
    dispatch(superstar_portfolio.getGroupInsiderList({ externalView, superstarGroupInsiderTradingSastUrl, corsKey })).then((response) => {
        ...
        Cell: ({ row }) => {
          return (
            <div
              ...
              className={` ${row.original.transaction_type_text == "Acquisition"
                  ? "positive_color"
